...
temp=[]
#input: timestamp | day's highest price, tab delimited '/\t'
#there are 20 rows in each input file with values missing

sample_input = input() #whole input without processing
try:
    while (sample_input != ''):
...
inputs = temp
db_len = inputs[0] #???  -- number of rows of data to follow? index of lines?
del inputs[0]      #???
...
for x in inputs:
    temp = x.split('\t')
    float_days = datetime.strptime(temp[0], '%m/%d/%Y %H:%M:%S') #transforming str to datetime
...
#output should contain exactly 20 rows of the missing values
